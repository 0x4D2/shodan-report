...
def analyze_trend(prev_snapshot: AssetSnapshot, current_snapshot: AssetSnapshot) -> str:
    ...
    if changes["new_ports"]:
        trend_lines.append(
            f"Neue offene Ports: {', '.join(map(str, changes['new_ports']))}"
        )

    if changes["removed_ports"]:
        trend_lines.append(
            f"Geschlossene Ports: {', '.join(map(str, changes['removed_ports']))}"
        )

    if changes["new_services"]:
        trend_lines.append(
            f"Neu entdeckte Dienste: {', '.join(changes['new_services'])}"
        )

    if changes["removed_services"]:
        trend_lines.append(
            f"Entfernte Dienste: {', '.join(changes['removed_services'])}"
        )

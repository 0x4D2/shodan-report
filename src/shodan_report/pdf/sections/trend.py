...
def create_trend_section(
    ...
    elements.append(
        Paragraph("<b>2. Trend- & Vergleichsanalyse</b>", styles["heading2"])
    )
...
def _add_comparison_view(
    elements: List, styles: Dict, trend_text: str, compare_month: str
) -> None:
    """Füge Trend-Ansicht MIT Monatsvergleich hinzu."""
    elements.append(
        Paragraph(f"<b>Veränderung zur {compare_month}-Analyse</b>", styles["normal"])
    )

...
def test_exposure_lower_when_ssh_is_vpn_protected():
    ...
    exposure_no_vpn = calculate_exposure_level(
        risk="MEDIUM",
        critical_points_count=0,
        open_ports=services_no_vpn
    )

    exposure_with_vpn = calculate_exposure_level(
        risk="MEDIUM",
        critical_points_count=0,
        open_ports=services_with_vpn
    )

    print(f"DEBUG: No VPN - Exposure: {exposure_no_vpn}")
    print(f"DEBUG: With VPN - Exposure: {exposure_with_vpn}")
    print(f"DEBUG: SSH ohne VPN - secure: {is_service_secure(services_no_vpn[0], ['ssh', 'https'])}")
    print(f"DEBUG: SSH mit VPN - secure: {is_service_secure(services_with_vpn[0], ['ssh', 'https'])}")
...
def test_empty_risk_string():
    exposure = calculate_exposure_level(
        risk="UNKNOWN",  # nicht in risk_boost
        critical_points_count=0,
        open_ports=[]
    )

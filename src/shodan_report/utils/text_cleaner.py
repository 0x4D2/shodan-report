...
class ReportTextCleaner:
    ...
    def remove_http_headers(text: str) -> str:
        ...
        http_header_prefixes = [
            ...
            'Connection:', 'ETag:', 'X-Powered-By:', 'Vary:', 
            'Last-Modified:', 'Accept-Ranges:', 'Transfer-Encoding:',
            'Location:', 'Set-Cookie:', 'Cache-Control:'
        ]
    ...
    def extract_key_information(text: str) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if 10 < len(sentence) < 60:
                ...
                if not any(sentence.startswith(prefix) for prefix in ['HTTP/', 'GET ', 'POST ', '{', '*']):
    ...
    @classmethod
    def clean_banner_for_report(cls, banner_text: str, product_name: Optional[str] = None) -> str:
        """
    ...
    def clean_critical_point(cls, point: str) -> str:
        ...
        product_match = re.match(r'^([A-Za-z0-9\.\s]+?)(?:\s+Version|\s+v\d|\s+on|\s+Port|$)', cleaned)

...
class RiskWeights:
    # Port Exposure
    open_ports: Dict[str, Any] = field(
        default_factory=lambda: {"thresholds": [15, 20, 30], "scores": [0, 1, 2, 3]}
    )

    # High-risk services
    high_risk_services: Dict[str, int] = field(
    ...
    # Server profiles
    server_profiles: Dict[str, Any] = field(
        default_factory=lambda: {
            "web_server": {
    ...
    # Version patterns
    vulnerable_indicators: Dict[str, int] = field(
        default_factory=lambda: {
            "1.0": 2,
            "2.0": 1,
            "deprecated": 3,
    ...
    # Secure indicators
    secure_indicators: List[str] = field(
        default_factory=lambda: ["tls", "ssl", "starttls", "https", "wss"]
    )
        ...
        with open(path, "r") as f:
            ...
            if "risk_weights" in config_data:
                if "open_ports" in config_data["risk_weights"]:
                    self.weights.open_ports.update(
                        config_data["risk_weights"]["open_ports"]
                    )

...
class CVEEvaluator(ServiceEvaluator):
    ...
    def evaluate(self, service: Service) -> ServiceRisk:
        ...
        critical_points = self._generate_detailed_critical_points(cve_counts, service, cves)
    ...
    def _generate_detailed_critical_points(self, cve_counts: Dict, service: Service, cves: List[CVE]) -> List[str]:
        ...
        # 1. Gesamt-CVE-Count
        if cve_counts["total"] > 0:
            critical_points.append(f"{product_info}{version_info} hat {cve_counts['total']} CVEs")
        
        # 2. Kritische CVEs
        if cve_counts["critical"] > 0:
            critical_points.append(f"{cve_counts['critical']} kritische CVEs (CVSS ≥ 9.0)")
        
        # 3. Hohe CVEs
        if cve_counts["high"] > 0:
            critical_points.append(f"{cve_counts['high']} hochriskante CVEs (CVSS 7.0-8.9)")
        ...
        if cves:
            ...
            if current_year in cve_years and cve_years[current_year] > 0:
                critical_points.append(f"{cve_years[current_year]} kritische CVEs aus {current_year}")
            
            # Zeige Top-Jahre
            if cve_years:
                top_years = sorted(cve_years.items(), key=lambda x: x[1], reverse=True)[:2]
                if len(top_years) > 0:
                    years_text = ", ".join(f"{year}: {count}" for year, count in top_years)
                    if len(cve_years) > 2:
        ...
        if cves and (cve_counts["critical"] > 0 or cve_counts["high"] > 0):
            ...
            if top_critical:
                ...
                if top_cve.cvss >= 9.0:
                    critical_points.append(f"Kritischste CVE: {top_cve.id} (CVSS {top_cve.cvss})")
    ...
    def _generate_recommendations(self, cve_counts: Dict, top_cves: List[CVE]) -> List[str]:
        ...
        if top_cves:
            top_cve = top_cves[0]
            if top_cve.cvss >= 9.0:
                recommendations.append(f"{top_cve.id}: Sofortiges Patching erforderlich")

...
class VersionEvaluator(ServiceEvaluator):
    ...
    def _analyze_version(self, product: str, version: str, service: Service) -> Dict[str, Any]:
        ...
        is_eol = self._is_version_eol(product, normalized_version, check_data["eol_versions"])
        ...
        if risk_score >= 2:  # Nur bei veralteten/kritischen Versionen
            ...
            if risk_score >= 4:
                recommendations.insert(0, f"SOFORT: {service.product} auf aktuelle Version updaten")
            if is_eol:
                recommendations.append(f"{service.product} Version {version} ist End-of-Life")
        ...
        if year_match:
            ...
            if year < 2020:
                return {
                    ...
                    "is_critical": False,
                    "recommendations": [f"{service.product} auf aktuelle Version prüfen"]
                }
    ...
    def _is_version_eol(self, product: str, version: str, eol_versions: List[str]) -> bool:
    ...
    def calculate_version_risk(self, product: str, version: str) -> int:
        ...
        version_info = self._analyze_version(product_key, version, 
                                           Service(port=0, transport="tcp", 

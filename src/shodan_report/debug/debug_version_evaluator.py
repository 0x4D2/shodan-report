...
if product_key in evaluator.VERSION_CHECKS:
    ...
    # Prüfe EOL
    is_eol = evaluator._is_version_eol(
        product_key, normalized_version, check_data["eol_versions"]
    )
    print(f"Is EOL: {is_eol}")

    # Prüfe critical
    is_critical = evaluator._compare_versions(
        normalized_version, "<=", check_data["critical_max"]
    )
    print(f"Is critical (<= {check_data['critical_max']}): {is_critical}")

    # Prüfe outdated
    is_outdated = evaluator._compare_versions(
